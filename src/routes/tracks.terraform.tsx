  ...
  head: () => ({
    meta: [
      { title: "Terraform IaC — DevOps By Nabawy" },
      {
        name: "description",
        content:
          "Read 5 TerraForm volumes as hosted PDFs, or preview the companion course site.",
      },
      { property: "og:title", content: "Terraform IaC — DevOps By Nabawy" },
      {
        property: "og:description",
        content: "Terraform library: foundations to interview arsenal, 5 volumes.",
      },
...
const OUTLINE = [
  { n: "Vol 1", title: "Foundations", text: "IaC, core loop, state, variables, modules." },
  { n: "Vol 2", title: "Production", text: "Multi-env, multi-VPC, for_each, ALB build." },
  { n: "Vol 3", title: "Practice Lab", text: "39 drills + bosses, warm-ups to final boss." },
  { n: "Vol 4", title: "Exam Center", text: "9 domains + 3 timed 57-question mocks." },
  { n: "Vol 5", title: "Interview Arsenal", text: "30-second answers, war stories, rubrics." },
];

type Tab = "pdfs" | "website";

function TerraformPage() {
  const track = tracks.find((item) => item.slug === "terraform");
  const hosted = booksByTrack("terraform");
  const totalPages = hosted.reduce((sum, book) => sum + book.pages, 0);
  const companionSites = companions["terraform"] ?? [];
  ...
  const downloadAll = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = "terraform-pdfs.zip";
  ...
  const tabs: { id: Tab; label: string; icon: typeof BookOpen; hint: string }[] = [
    { id: "pdfs", label: "PDF volumes", icon: BookOpen, hint: `${hosted.length}` },
